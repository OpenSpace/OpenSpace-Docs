...
## Texture Map (.tmap)
Texture maps are used when loading CSV datasets that should be [rendered using multiple textures](./advanced.md#using-multiple-textures). The dataset should then include a column with indices of textures to use for the rendering, and the texture map file is a mapping between those indices and a certain texture file.
